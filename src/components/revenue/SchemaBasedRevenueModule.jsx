 ...
 * Displays revenue items from the schema and allows adjusting them with sliders
 */
const SchemaBasedRevenueModule = () => {
  // Get schema data
  ...
  // Get all revenue items from the schema
  const revenueItems = useMemo(() => {
    if (!schemaData || !schemaData.items) return {};
    
    // Group revenue items by subcategory
    const grouped = {};
  ...
  const defaultRevenueValues = useMemo(() => {
    const defaults = {};
    
    if (schemaData && schemaData.items) {
      Object.values(schemaData.items).forEach(item => {
        if (item.category === 'revenue' && item.yearData) {
          defaults[item.id] = { 
  ...
  // Handle slider change for a revenue item value (min/max)
  const handleRevenueValueChange = useCallback((itemId, yearKey, isMin, value) => {
  ...
  // Handle quantity change for a revenue item
  const handleQuantityChange = useCallback((itemId, yearKey, quantity) => {
    ...
    // Also update the revenue values based on the quantity and per-unit values
    if (metadata.perUnitRevenue) {
      const yearData = { ...item.yearData };
      const perUnitMin = metadata.perUnitRevenue.min || 0;
      const perUnitMax = metadata.perUnitRevenue.max || 0;
  ...
  // Handle growth percentage change for a revenue item
  ...
  // Reset all revenue values for the current year to their defaults
  const resetRevenueForYear = useCallback(() => {
    if (!schemaData || !schemaData.items) return;
    
    Object.values(schemaData.items).forEach(item => {
      if (item.category === 'revenue' && defaultRevenueValues[item.id]) {
        // Get default values for this item
        const defaultValues = defaultRevenueValues[item.id];
  ...
  }, [schemaData, defaultRevenueValues, selectedYear, updateSchemaItem]);
  
  // Debounced version of handlers
  const debouncedHandleRevenueValueChange = useDebouncedCallback(handleRevenueValueChange, 300);
  ...
  const yearTotals = useMemo(() => {
    ...
    Object.values(schemaData.items).forEach(item => {
      if (item.category === 'revenue' && item.yearData && item.yearData[selectedYear]) {
  ...
  React.useEffect(() => {
    registerModuleCalculation('schemaRevenue', () => {
      const revenueByYear = {};
      ...
      yearRange.forEach(year => {
        ...
        Object.values(schemaData.items).forEach(item => {
          if (item.category === 'revenue' && item.yearData && item.yearData[year]) {
            minTotal += item.yearData[year].min;
            maxTotal += item.yearData[year].max;
            
            // Add to breakdown by subcategory
            if (!revenueBreakdown[item.subcategory]) {
              revenueBreakdown[item.subcategory] = { min: 0, max: 0 };
            }
            
            revenueBreakdown[item.subcategory].min += item.yearData[year].min;
            revenueBreakdown[item.subcategory].max += item.yearData[year].max;
          }
        });
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-gray-800">Revenue Scenario Planner</h2>
      ...
      {resetConfirmOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full">
            <h3 className="text-lg font-bold text-gray-800 mb-3">Reset Revenue Values</h3>
            <p className="text-gray-600 mb-4">
              Are you sure you want to reset all revenue values for {selectedYear} to their defaults?
      ...
      <div className="mb-4 p-4 bg-green-50 rounded-md">
        <div className="flex justify-between items-center">
          ...
          <div className="text-right">
            <p className="text-sm text-green-500">Range:</p>
      ...
      <div className="space-y-6">
        {Object.keys(revenueItems).map(subcategory => {
          const items = revenueItems[subcategory];
                    ...
                    return (
                      <div key={item.id} className="border-b border-gray-100 pb-6 last:border-b-0 last:pb-0">
                        ...
                        <OptimizedRevenueSliders
                          ...
                          onValueChange={debouncedHandleRevenueValueChange}
...
export default SchemaBasedRevenueModule; 